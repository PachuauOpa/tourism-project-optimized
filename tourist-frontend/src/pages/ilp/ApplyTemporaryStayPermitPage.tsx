...
import { TemporaryStayPermitForm, IlpSubmissionResponse } from '../../types';
...
const ApplyTemporaryStayPermitPage: React.FC = () => {
  const navigate = useNavigate();
  const [formData, setFormData] = useState<TemporaryStayPermitForm>({
  ...
  const [errors, setErrors] = useState<Partial<TemporaryStayPermitForm>>({});
  ...
  const updateFormData = (field: keyof TemporaryStayPermitForm, value: string | File | null): void => {
  ...
  const handleSubmit = async (e: React.FormEvent): Promise<void> => {
    ...
    try {
      const uploadedDocument = uploadDocumentFile
        ? await uploadDocumentToMizTour(uploadDocumentFile, 'temporary-stay-permit')
        : null;

      const result = await postForm('/api/forms/temporary-stay-permit', {
        ...rest,
  ...
  return (
    <Screen className="ilp-application-screen">
      <div className="ilp-application-header">
        ...
        <h1 className="ilp-form-title">Apply Temporary Stay Permit</h1>
        <p className="ilp-form-subtitle">( 30 Days Validity )</p>
      </div>
            ...
            <div className="form-grid">
              <div className="ilp-form-field">
                ...
                <select value={formData.selectType} onChange={(e) => updateFormData('selectType', e.target.value)} required>
                  ...
                  <option value="Govt / Private / PSU Related">Govt / Private / PSU Related</option>
                </select>
              </div>
...
export default ApplyTemporaryStayPermitPage;
