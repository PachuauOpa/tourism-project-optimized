...
import { AutoGraphRounded as AutoGraphRoundedIcon, ArticleRounded as ArticleRoundedIcon, SearchRounded as SearchRoundedIcon, NotificationsActiveRounded as NotificationsActiveRoundedIcon, RemoveRedEyeRounded as RemoveRedEyeRoundedIcon, EditRounded as EditRoundedIcon } from '@mui/icons-material';
import {
  Area,
  AreaChart,
  Bar,
...
const IlpAdminDashboardPage: React.FC = () => {
  ...
  const chartAccent = isLightTheme ? '#90CEFF' : '#59b7ff';
  const chartGridColor = isLightTheme ? '#d6e2f1' : '#2b3c56';
  const chartTickColor = isLightTheme ? '#64748b' : '#c2d2e8';
  const chartTrackColor = isLightTheme ? '#edf3fb' : '#1d2737';
  const mapTileUrl = isLightTheme
  ...
  const navItems: Array<{ icon: React.ElementType; label: string; view: ViewMode }> = [
    { icon: AutoGraphRoundedIcon, label: 'Dashboard', view: 'analytics' },
    { icon: ArticleRoundedIcon, label: 'Applications', view: 'applications' }
  ];
  ...
  return (
    <div className={`admin-page admin-theme-${theme}`}>
      ...
      <div className="admin-main-wrapper">
        ...
        <header className="ilp-admin-panel" style={{ padding: '24px 32px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <div className="ilp-admin-search-wrap">
            <span className="search-icon"><SearchRoundedIcon fontSize="small" /></span>
            <input
              type="text"
          ...
          {activeView === 'analytics' && (
            <>
              ...
              {/* Refactored Dashboard Analytics Layout */}

              {/* Top Row: Full Width Timeline */}
              <div className="ilp-admin-fw-row">
                  ...
                  <div className="ilp-admin-chart-wrap">
                    <ResponsiveContainer width="100%" height={250}>
                      <AreaChart data={timeline}>
                        ...
                        <XAxis dataKey="month" tick={{ fontSize: 11, fontWeight: 700, fill: chartTickColor }} />
                        <YAxis allowDecimals={false} tick={{ fontSize: 11, fontWeight: 700, fill: chartTickColor }} />
                        <Tooltip />
                        <Area dataKey="count" stroke={chartAccent} strokeWidth={2} fill="url(#timelineGrad)" />
                      </AreaChart>
                    </ResponsiveContainer>
                  </div>
                </div>
              ...
              {/* Middle Row: 3-Column Chart Distribution */}
              <div className="ilp-admin-tri-col">
                ...
                <div className="ilp-admin-panel">
                  ...
                  <div className="ilp-admin-donut-wrap">
                    <DonutChart data={typeBreakdown} total={metrics.total} trackColor={chartTrackColor} />
                  ...
                  <div className="ilp-admin-chart-wrap">
                    <ResponsiveContainer width="100%" height={250}>
                      <BarChart data={statusBreakdown}>
                        <CartesianGrid strokeDasharray="4 4" stroke={chartGridColor} />
                        <XAxis dataKey="name" tick={{ fontSize: 11, fontWeight: 700, fill: chartTickColor }} />
                        <YAxis allowDecimals={false} tick={{ fontSize: 11, fontWeight: 700, fill: chartTickColor }} />
                        <Tooltip />
                  ...
                  <div className="ilp-admin-chart-wrap">
                    <ResponsiveContainer width="100%" height={250}>
                      <BarChart data={purposeBreakdown} layout="vertical">
                        <CartesianGrid strokeDasharray="4 4" stroke={chartGridColor} />
                        <XAxis type="number" tick={{ fontSize: 11, fontWeight: 700, fill: chartTickColor }} />
                        <YAxis dataKey="name" type="category" width={120} tick={{ fontSize: 11, fontWeight: 700, fill: chartTickColor }} />
                        <Tooltip />
                        <Bar dataKey="value" fill={chartAccent} radius={[0, 10, 10, 0]} />
                      </BarChart>
                    </ResponsiveContainer>
                  </div>
                </div>
              ...
              {/* Bottom Row: Map and Geo Metrics sidebar */}
              <div className="ilp-admin-geo-row">
                {/* Left Side: Map */}
                  ...
                  >
                    ...
                    {mapPoints.map((point) => (
                      <LeafletCircleMarker
                        ...
                        pathOptions={{ color: chartAccent, fillColor: chartAccent, fillOpacity: 0.7 }}
                      >
                        <Popup>
              ...
              {/* Right Side: Geo Metrics */}
              <div className="ilp-admin-geo-sidebar">
                {/* Top Countries */}
                <div className="ilp-admin-panel">
                  <div className="ilp-admin-panel-header">
                    <h2>Top Countries</h2>
                    ...
                    {paginatedApplicants.map((item) => (
                      <tr key={`${item.application_type}-${item.id}`}>
                        ...
                        <td>
                          ...
                          <button type="button" className="ilp-admin-action-btn" title="Edit" style={{ marginLeft: 8 }}>
